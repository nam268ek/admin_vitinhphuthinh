...
import { getAddToCartService } from '../../redux/Slices/OrderSlice';

export const ModalListProductsV2: React.FC<any> = ({ open, setOpen }) => {
  ...
  const handleAddToCart = async (e: any, id: string) => {
    const isExist = cartItem?.filter((item) => item.id === id).length > 0;
    if (isExist) {
      openMessage('Sản phẩm đã có trong giỏ hàng');
      return;
    }
    try {
      await dispatch(
  ...
  return (
    <Modal width={'80%'} open={open} onCancel={handleCancel} confirmLoading={loading} footer={null}>
          ...
          {products?.map((item, index) => (
            <Card
              ...
              cover={
                <Image
                  alt="example"
                  // src="error"
                  src={item?.images[0]?.thumbUrl || 'error'}
                  width={200}
                  height={150}
              ...
              actions={[
                <ShoppingCartOutlined key="cart" onClick={(e) => handleAddToCart(e, item.id)} />,
            ...
            >
              <Meta
                ...
                description={
                  <div>
                    <p className="m-0">Giá bán: {formatMoney.format(Number(item?.priceSale))}</p>
                    <p className="m-0">Kho: {item?.quantity} sản phẩm</p>
                  </div>
                }
              />
            </Card>
